...
def make_exe():
    ...
    python_config.run_command = "from theapp import cli; cli()"

    # Run a Python module as __main__ when the interpreter starts.
    # python_config.run_module = "<module>"

    # Run a Python file when the interpreter starts.
    # python_config.run_filename = "__main__.py"
    ...
    for resource in exe.pip_install(["git+https://github.com/openai/whisper.git"]):
        resource.add_location = "filesystem-relative:lib"
        exe.add_python_resource(resource)
    for resource in exe.pip_install(['./pyapp']):
        resource.add_location = "filesystem-relative:lib"
        # resource.add_location = "in-memory"
        exe.add_python_resource(resource)
    # exe.add_python_resources(exe.read_package_root(path="pyapp/", packages=['theapp']))
